...
// File contracts/PersonalUSDTBatch.sol
...
/// @title Personal USDT batch payments on Ethereum mainnet
/// @notice Only the owner can distribute their own approved USDT. A failed
/// payment reverts the entire batch. Amounts are in USDT's six-decimal units.
contract PersonalUSDTBatch is Ownable2Step, ReentrancyGuard {
    ...
    /// @notice Recover ERC-20 tokens accidentally sent to this contract, to owner only.
    /// @dev This does not spend anyone's wallet allowance.
    ...
    /// @notice Ownership can move through the inherited two-step process, but
    /// cannot be abandoned accidentally, which would disable all owner functions.
