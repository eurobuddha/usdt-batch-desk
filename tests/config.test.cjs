...
test('unconfigured contract cannot prepare spending',async()=>{let calls=0;const injected={request:async()=>{calls++;throw new Error('Unexpected wallet call')}};await assert.rejects(()=>readWallet(injected,''),/Choose a shared/);assert.equal(calls,0)});
